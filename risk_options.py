...
_BUILD = "2026-09-07.2"   # shown in the tab — bump when this module changes
...
# (Δ×F×mult×lots — pricer caption + empirical probe 2026-09-07), unlike
# equities/commod where it is $/pt. Branch on this set to avoid the double-F
# bug (FX option Δ-est ~150× too small; delta-mode VaR/exposures likewise).
    ...
    for o in opts:
        try:
            ...
            if o["src"] == "v2" and o["mkt"].split("_")[0] in _FX_V2:
                ...
                if not F0:
                    continue
                pdelta = pdelta / F0        # FX: notional → $/pt of the future
            ...
            continue
    if out:                # don't memoize a fully-failed pass (a transient yf
...
def compute(book: pd.DataFrame, mode: str, products: dict, ivols: dict,
    ...
    for o in opts:
        ...
        if prod == "Rates":
            ...
            risk_de = abs(d_usd) * F0                  # $/pt × F = $ notional
            de_txt = f"${risk_de:,.0f}"
